...
// https://gist.github.com/ryancdotorg/5766644037db6b96ae63a79a2faaf784
...
function salsa20_8(B: TypedArray, x: TypedArray) {
...
function blockxor(S: TypedArray, Si: number, D: TypedArray, len: number) {
  for (let i = 0; i < len; i++) {
    D[i] ^= S[Si + i];
  }
}
...
function checkBufferish(o: Uint8Array | ArrayBuffer): boolean {
 ...
 * @param N CPU cost
 * @param r Memory cost
 * @param p Parallelization cost
 * @param dkLen Key length
 * @param callback
 * @returns Promise of the encrypted password
 */
async function scrypt(
  ...
  const bBuf = await PBKDF2_HMAC_SHA256(password, salt, 1, p * 128 * r);
  // SubtleCrypto gives an ArrayBuffer, we need a Uint8Array.
  let b = new Uint8Array(bBuf);
  ...
  // stop and give other events on the event loop a chance to run. ~RicMoo
  const incrementalSMix = async function () {
    if (stop) {
      callback?.(new Error("cancelled"), currentOp / totalOps);
      return;
    }
  ...
  do { // Run the smix state machine until completion
    derivedKey = incrementalSMix();
  } while (typeof (await derivedKey) === "undefined");

  return derivedKey;
}

export { PBKDF2_HMAC_SHA256 as pbkdf2, scrypt };
